...
def processCommand(command, queue):
    if command == "Siguiente":
        try:
            value = queue.pop()  # Utilizar el método pop() de LinkedList para obtener el siguiente elemento
            print(value)
        except Exception as e:
            print("No hay fila")
    else:
        queue.append(command)  # Utilizar append() de LinkedList para agregar un elemento a la fila
def main():
    cases = int(stdin.readline().strip())
    for case in range(cases):
        queue = LinkedList()
        n_instructions = int(stdin.readline().strip())
        for intruction in range(n_instructions):
